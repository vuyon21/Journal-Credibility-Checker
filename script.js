...
/* Map accredited row variants to unified object */
function mapAccreditedRow(cols, header){
  ...
  const lastReview = get(['DATE OF LAST REVIEW OR ACCREDITATION','Date of Last Review or Accreditation','LastReview','AccreditedDate']) || '';
  const international = get(['International Accreditation','International','Index']) || '';
  const frequency = get(['FREQUENCY','Frequency']) || '';
  const publisher = get(['Publisher','Publisher details','Publisherdetails','Publisher’sdetails']) || fallbackPublisher;
...
/* Map transformative row variants */
function mapTransformRow(cols, header){
  const h = header.map(x => String(x || '').trim());
  const get = (names) => {
    const idx = h.findIndex(hname => names.some(n => hname.replace(/\s+/g,'').toLowerCase() === n.replace(/\s+/g,'').toLowerCase()));
    return (idx >= 0 && cols[idx]) ? cols[idx].trim() : '';
  ...
  const title = get(['Journal Title','JournalTitle','Title','Journal']) || fallbackTitle;
  const eissn = get(['eISSN','EISSN','OnlineISSN']) || fallbackEISSN;
  const oaStatus = get(['Open Access Status','OpenAccessStatus','OpenAccess','OAStatus']) || '';
  const included = get(['Included in R&P agreement','IncludedinR&Pagreement','Included','IncludedinRPagreement']) || '';
  const explanation = get(['Explanation if not included','Explanationifnotincluded','Notes']) || '';
  const subject = get(['Subject']) || '';
  const publisher = get(['Publisher','Publisher details','Publisherdetails']) || fallbackPublisher;
  const duration = get(['Agreement Duration','Duration']) || '';
...
function mapRemovedRow(cols, header){
  ...
  const fallbackTitle = cols[0] || '';
  const fallbackISSN = cols[1] || '';
  const title = get(['JOURNALTITLE(Previoustitleifapplicable)','Journal title','JournalTitle','Title']) || fallbackTitle;
  const issn = get(['ISSN','ISSN(Online)','eISSN']) || fallbackISSN;
  const reason = get(['Reason','REASON','Comment']) || '';
  const review = get(['DATE OF LAST REVIEW OR ACCREDITATION','Reviewed','Reviewdate','Review/Year']) || '';
  const publisher = get(['Publisher','Publisher details','Publisherdetails']) || '';
  ...
  for(const f of ACCREDITED_FILES){
    try{
      ...
      for(const cols of dataRows){
        ...
        const mapped = mapAccreditedRow(cols, header.length ? header : cols);
...
async function loadRemoved(){
  ...
  try{
    ...
    for(const cols of dataRows){
      if(!cols.some(c => String(c || '').trim())) continue;
      out.push(mapRemovedRow(cols, header.length ? header : cols));
    }
...
async function loadTransformative(){
  ...
  for(const f of TRANSFORMATIVE_FILES){
    try{
      ...
      for(const cols of dataRows){
        if(!cols.some(c => String(c || '').trim())) continue;
        const mapped = mapTransformRow(cols, header.length ? header : cols);
        if(mapped.title) results.push(mapped);
      }
...
async function doSearch(rawQuery){
  ...
  const idPublisher = accreditedHits[0]?.publisher || live.crossref?.publisher || live.openalex?.publisher || '';
  ...
  // Recommendation
  parts.push({ title: 'Recommendation', rows: [[recText, '']] });
...
/* ================== Autocomplete setup (already called in init) ================== */
/* (setupAutocomplete used earlier) -- already wired in init() */
